        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '上班签到状态' : ''">
                        <el-select v-model="searchForm.qiandaoShangbanTypes" placeholder="请选择上班签到状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in qiandaoShangbanTypesSelectSearch"
                               v-bind:key="index"
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '下班签退状态' : ''">
                        <el-select v-model="searchForm.qiandaoXiabanTypes" placeholder="请选择下班签退状态">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in qiandaoXiabanTypesSelectSearch"
                               v-bind:key="index"
                               :label="item.indexName"
                               :value="item.codeIndex">
                            <!--lable是要展示的名称-->
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/kaoqinguanli/upload/qiandaoMuBan.xls"
                        >批量导入员工签到数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('qiandao','导入导出')"
                            ...
                            >批量导入员工签到数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('qiandao','导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "qiandao.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="qiandaoShangbanTime"
                                   header-align="center"
                                   label="上班签到时间">
                        <template slot-scope="scope">
                            {{scope.row.qiandaoShangbanTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="qiandaoShangbanTypes"
                                      header-align="center"
                                      label="上班签到状态">
                        <template slot-scope="scope">
                            {{scope.row.qiandaoShangbanValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="qiandaoXiabanTime"
                                   header-align="center"
                                   label="下班签退时间">
                        <template slot-scope="scope">
                            {{scope.row.qiandaoXiabanTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="qiandaoXiabanTypes"
                                      header-align="center"
                                      label="下班签退状态">
                        <template slot-scope="scope">
                            {{scope.row.qiandaoXiabanValue}}
                        </template>
                    </el-table-column>
...
<script>
    ...
    //当前表下拉框搜索条件
              qiandaoShangbanTypesSelectSearch : [],
              qiandaoXiabanTypesSelectSearch : [],
            form:{
                id : null,
                yuangongId : null,
                qiandaoShangbanTime : null,
                qiandaoShangbanTypes : null,
                qiandaoXiabanTime : null,
                qiandaoXiabanTypes : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '上班签到时间': "qiandaoShangbanTime",
                     '上班签到状态': "qiandaoShangbanValue",
                     '下班签退时间': "qiandaoXiabanTime",
                     '下班签退状态': "qiandaoXiabanValue",
                     '所属日期': "insertTime",
            },
        ...
        methods: {
            ...
            getDataList() {
                ...
                if (this.searchForm.qiandaoShangbanTypes!= '' && this.searchForm.qiandaoShangbanTypes!= undefined) {
                    params['qiandaoShangbanTypes'] = this.searchForm.qiandaoShangbanTypes
                }
                 
                if (this.searchForm.qiandaoXiabanTypes!= '' && this.searchForm.qiandaoXiabanTypes!= undefined) {
                    params['qiandaoXiabanTypes'] = this.searchForm.qiandaoXiabanTypes
                }
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=qiandao_shangban_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.qiandaoShangbanTypesSelectSearch = data.data.list;
                    }
                });
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=qiandao_xiaban_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.qiandaoXiabanTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
...
//                            qiandaoTypes:qiandaoTypes,
          ...
          qiandao(zhuangtai) {
                ...
                _this.$confirm(msg, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "qiandao/qiandao?zhuangtai="+zhuangtai,
                        method: "get",
                        data: {
                        }
                    }).then(({data}) => {
