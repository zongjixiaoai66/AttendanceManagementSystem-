    ...
    <div class="addEdit-block">
        <el-form
            ...
            <input id="yuangongId" name="yuangongId" type="hidden">
                <el-col :span="12">
                    <el-form-item v-if="type!='info' && !ro.qiandaoShangbanTime"  class="input" label="上班签到时间" prop="qiandaoShangbanTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.qiandaoShangbanTime"
                                type="datetime"
                                placeholder="上班签到时间"
                                :disabled="ro.qiandaoShangbanTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.qiandaoShangbanTime" label="上班签到时间" prop="qiandaoShangbanTime">
                            <span v-html="ruleForm.qiandaoShangbanTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="上班签到状态" prop="qiandaoShangbanTypes">
                        <el-select v-model="ruleForm.qiandaoShangbanTypes" :disabled="ro.qiandaoShangbanTypes" placeholder="请选择上班签到状态">
                            <el-option
                                v-for="(item,index) in qiandaoShangbanTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="上班签到状态" prop="qiandaoShangbanValue">
                        <el-input v-model="ruleForm.qiandaoShangbanValue"
                            placeholder="上班签到状态" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type!='info' && !ro.qiandaoXiabanTime"  class="input" label="下班签退时间" prop="qiandaoXiabanTime">
                        <el-date-picker
                                value-format="yyyy-MM-dd HH:mm:ss"
                                v-model="ruleForm.qiandaoXiabanTime"
                                type="datetime"
                                placeholder="下班签退时间"
                                :disabled="ro.qiandaoXiabanTime">
                        </el-date-picker>
                    </el-form-item>
                    <div v-else-if="type=='info'">
                        <el-form-item v-if="ruleForm.qiandaoXiabanTime" label="下班签退时间" prop="qiandaoXiabanTime">
                            <span v-html="ruleForm.qiandaoXiabanTime"></span>
                        </el-form-item>
                    </div>
                </el-col>

                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="下班签退状态" prop="qiandaoXiabanTypes">
                        <el-select v-model="ruleForm.qiandaoXiabanTypes" :disabled="ro.qiandaoXiabanTypes" placeholder="请选择下班签退状态">
                            <el-option
                                v-for="(item,index) in qiandaoXiabanTypesOptions"
                                v-bind:key="item.codeIndex"
                                :label="item.indexName"
                                :value="item.codeIndex">
                            </el-option>
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="下班签退状态" prop="qiandaoXiabanValue">
                        <el-input v-model="ruleForm.qiandaoXiabanValue"
                            placeholder="下班签退状态" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
            </el-row>
    ...
    export default {
        data() {
            return {
                ...
                ro:{
                    yuangongId: false,
                    qiandaoShangbanTime: false,
                    qiandaoShangbanTypes: false,
                    qiandaoXiabanTime: false,
                    qiandaoXiabanTypes: false,
                    insertTime: false,
                },
                ruleForm: {
                    yuangongId: '',
                    qiandaoShangbanTime: '',
                    qiandaoShangbanTypes: '',
                    qiandaoXiabanTime: '',
                    qiandaoXiabanTypes: '',
                    insertTime: '',
                },
                qiandaoShangbanTypesOptions : [],
                qiandaoXiabanTypesOptions : [],
                yuangongOptions : [],
                rules: {
                   yuangongId: [
                   ...
                   qiandaoShangbanTime: [
                              { required: true, message: '上班签到时间不能为空', trigger: 'blur' },
                          ],
                   qiandaoShangbanTypes: [
                              { required: true, message: '上班签到状态不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
                   ...
                   qiandaoXiabanTime: [
                              { required: true, message: '下班签退时间不能为空', trigger: 'blur' },
                          ],
                   qiandaoXiabanTypes: [
                              { required: true, message: '下班签退状态不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=qiandao_shangban_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.qiandaoShangbanTypesOptions = data.data.list;
                    }
                });
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=qiandao_xiaban_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.qiandaoXiabanTypesOptions = data.data.list;
                    }
                });
